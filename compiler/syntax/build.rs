...
impl<'a> Generator<'a> {
	...
	fn write(out: &Path, file: &str, uses: Option<&str>, content: String) {
		let mut text = "// This file is generated by build.rs\n// Do not edit\n\n".to_string();
	...
	fn generate(mut self, out: &Path) {
		...
		Self::write(out, "token.rs", Some("use crate::generated::*;"), self.gen_tokens());
		Self::write(
			out,
			"ast.rs",
	...
	fn gen_kinds(&mut self) -> String {
		...
		let token_display = token_kinds.iter().map(|(x, _)| x);
		let token_kinds = token_kinds.iter().map(|(_, x)| format_ident!("{}", x));
		let node_display = node_kinds.iter();
		let node_kinds = node_kinds.iter().map(|x| format_ident!("{}", x));

		let tk = token_kinds.clone();
		let nk = node_kinds.clone();
		let tk2 = token_kinds.clone();
		let tk3 = token_kinds.clone();
		...
		let from = quote! {
			impl From<lex::token::TokenKind> for SyntaxKind {
				fn from(kind: lex::token::TokenKind) -> Self {
					match kind {
						#(lex::token::TokenKind::#tk2 => Self::#tk3,)*
								...
								fn cast(node: SyntaxNode) -> Option<Self> {
									node.children_with_tokens().find_map(|x| match x {
										SyntaxElementRef::Node(node) => match node.kind() {
											#(SyntaxKind::#struct_variants => #struct_variants::cast(node.clone()).map(Self::#struct_variants),)*
											_ => {
												None #(.or_else(|| #enum_variants::cast(node.clone()).map(Self::#enum_variants)))*
											},
										},
...
fn map_token(x: &str) -> (String, String) {
	match x {
		...
		x @ "op" => (x.to_string(), "Operator".to_string()),
		x => {
